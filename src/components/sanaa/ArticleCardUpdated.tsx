...
import { Clock, Bookmark, BookmarkCheck, User } from 'lucide-react'
...
export function ArticleCard({ article, variant = 'default' }: ArticleCardProps) {
  const { openArticle, bookmarks, toggleBookmark } = useStore()
  ...
  if (variant === 'horizontal') {
    return (
      <Link href={`/articles/${article.slug}`} onClick={() => openArticle(article)} className="w-full flex gap-4 text-left group">
  ...
  if (variant === 'compact') {
    return (
      <Link href={`/articles/${article.slug}`} onClick={() => openArticle(article)} className="w-full text-left p-3 rounded-lg hover:bg-secondary/50 transition-colors group">
        <div className="flex items-start gap-3">
          <span className="font-mono text-xs text-muted-foreground mt-0.5">{article.category.name}</span>
          <div className="flex-1 min-w-0">
            <p className="font-serif font-semibold text-sm group-hover:text-primary transition-colors line-clamp-1">{article.title}</p>
            <p className="text-xs text-muted-foreground mt-1 font-mono">{article.readTime} min read</p>
          </div>
        </div>
      </Link>
    )
  ...
  return (
    <article className="group cursor-pointer" onClick={() => openArticle(article)}>
      <div className="relative rounded-xl overflow-hidden mb-3 aspect-[16/10] bg-secondary">
        ...
        <Badge className="absolute top-3 left-3 text-xs" style={{ backgroundColor: article.category.color, color: '#fff' }}>
          {article.category.name}
        </Badge>
        <Button
          variant="ghost"
        ...
        >
          {isBookmarked ? <BookmarkCheck className="h-3.5 w-3.5 text-gold" /> : <Bookmark className="h-3.5 w-3.5" />}
        </Button>
      </div>
      ...
      <div className="flex items-center gap-3 text-xs text-muted-foreground">
        <span className="font-medium text-foreground hover:text-primary transition-colors">{article.author.name}</span>
        <span className="text-border">·</span>
        <span>{new Date(article.publishedAt).toLocaleDateString('en-KE', { day: 'numeric', month: 'short', year: 'numeric' })}</span>
